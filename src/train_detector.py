...
def process_feature_engineering(beacon_df: pd.DataFrame):
    ...
    beacon_df.drop(columns=['event_id', 'timestamp', 'src_ip', 'dst_ip', 'signed_binary', 'host_id'], inplace=True)
    # Convert categorical columns to category dtype
    categorical_cols = ['dst_port', 'proc_name', 'wierdness', 'proc_risk']
    ...
    categorical_cols = ['proc_name', 'dst_port', 'protocol', 'country_code', 'user', 'wierdness', 'proc_risk']
...
def train_supervised_models(beacon_df: pd.DataFrame):
    ...
    #beacon_df = pd.read_csv(r'C:\Users\Soumendra\Documents\GitHub\BeaconHunter\data\beacon_events_train.csv')
    # fill missing values in inter_event_seconds with median as this is the only feature which has missij=ng values
    # in the dataset
    beacon_df['inter_event_seconds'] = beacon_df['inter_event_seconds'].fillna(beacon_df['inter_event_seconds'].median())

    #process_feature_engineering(beacon_df)
    beacon_df = create_derived_features(beacon_df)
...
def train_unsupervised_model(beacon_df: pd.DataFrame):
    # Implement unsupervised model Isolation Forest
    # Define isolation forest model parameters
...
def main():
    ...
    df = pd.read_csv(r'C:\Users\Soumendra\Documents\GitHub\BeaconHunter\data\beacon_events_train.csv')
    calculate_fusion_risk_scores(beacon_df_org=df)
